...
class SslController extends CheckoutBaseControlller
{
    public function store(Request $request)
    {
        ...
        $order['txnid'] = "SSLCZ_TXN_".uniqid();
        $cancel_url = route('front.payment.cancle');
        $notify_url = route('front.ssl.notify');

        Session::put('input_data',$input);
        Session::put('order_data',$order);
        Session::put('order_payment_id', $order['txnid']);
        ...
        Session::put('tcart',utf8_encode(bzcompress(serialize($cart), 9)));
        $s_datas = Session::all();
        $session_datas = json_encode($s_datas);
        file_put_contents(storage_path().'/ssl/'.$order['txnid'].'.json', $session_datas); 
    ...
    public function notify(Request $request)
    {
        ...
        if(file_exists(storage_path().'/ssl/'.$input_data['tran_id'].'.json')){
            $data_results = file_get_contents(storage_path().'/ssl/'.$input_data['tran_id'].'.json');
            $lang = json_decode($data_results, true);
            foreach($lang as $key => $lan){
                Session::put(''.$key,$lan);
            }
            unlink(storage_path().'/ssl/'.$input_data['tran_id'].'.json');
        }
        ...
        if($input_data['status'] == 'VALID'){
                ...
                $cart = unserialize(bzdecompress(utf8_decode(Session::get('tcart'))));
                ...
                Session::put('tempcart',unserialize(bzdecompress(utf8_decode(Session::get('tcart')))));
                Session::forget('cart');
