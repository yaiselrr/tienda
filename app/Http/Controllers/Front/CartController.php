...
class CartController extends FrontBaseController
{
   ...
   public function addcart($id)
    {    
        ...
        $prod = Product::where('id','=',$id)->first(['id','user_id','slug','name','photo','size','size_qty','size_price','color','minimum_qty','price','stock','type','file','link','license','license_qty','measure','whole_sell_qty','whole_sell_discount','attributes','minimum_qty']);
        ...
        $cart->add($prod, $prod->id,$size,$color,$keys,$values,$qty);
        ...
        $data[1] = round($data[1] * $curr->value,2);
        if($this->gs->currency_format == 0){
            $data[1] = $curr->sign.$data[1];
        }
        else{
            $data[1] = $data[1].$curr->sign;
        }
