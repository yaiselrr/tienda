        ...
        @if(Auth::check())
            ...
            <div class="wallet-box mt-3 d-none">
                ...
                <form id="wallet-form">
                    ...
                    <input type="number" placeholder="{{ __("Enter Your Amount") }}" step="0.01" id="wallet-amount" min="1" required="" value="{{ $totalPrice <= ( Auth::user()->balance * $curr->value ) ? $totalPrice :  round(Auth::user()->balance * $curr->value ,2) }}">

                    @elseif(Session::has('coupon_total1'))

                    <input type="number" placeholder="{{ __("Enter Your Amount") }}" step="0.01" id="wallet-amount" min="1" required="" value="{{ preg_replace("/[^0-9,.]/", "", Session::get('coupon_total1')) <= ( Auth::user()->balance * $curr->value ) ? preg_replace("/[^0-9,.]/", "", Session::get('coupon_total1')) :  round(Auth::user()->balance * $curr->value ,2) }}">

                    @else

                    <input type="number" placeholder="{{ __("Enter Your Amount") }}" step="0.01" id="wallet-amount" min="1" required="" value="{{ round($totalPrice * $curr->value,2) <= ( Auth::user()->balance * $curr->value ) ? round($totalPrice * $curr->value,2) :  round(Auth::user()->balance * $curr->value ,2) }}">
            ...
            $.ajax({
                    ...
                    success:function(data){
                        if(data == 0){
							...
							if(data[0] == 0){
								$('.checkoutform').prop('action','{{ route('front.wallet.submit') }}');
